...
const all = 999999999999999;
let AVAILABLE_AMOUNTS = [5, 10, 20];
...
const logOverviewScreen = (model) => {
  ...
  return h("", { onremove: () => model.logs.clearLogs() }, [
    ...
    h(".flex-row.justify-between.items-center", [
      h("h2.mv2", "Logs"),
      h(
        "button.btn.btn-primary.w-15.h2#create",
        {
    ...
    h(".flex-row.mv1", [
      ...
      h(
        ".w-85",
        data.isSuccess() &&
          h(".w-100.flex-column.mh3", [
            table(
              data.isSuccess() ? data.payload : [],
              logsColumns,
              model,
              (entry) => ({
                onclick: () =>
                  model.router.go(`?page=log-detail&id=${entry.id}`),
              })
            ),
            h(".flex-row.justify-between.mv3", [
              ...
              pageSelector(totalPages, selectedPage, (page) =>
                model.logs.setSelectedPage(page)
              ),
