...
const pageSelector = (pageCount, selectedPage, onclick) => {
  ...
  if (pageCount <= MAX_VISIBLE_PAGES) {
    ...
    const frontBound = Math.min(
      Math.max(0, selectedPage - MAX_VISIBLE_PAGES + 2),
      pageCount - MAX_VISIBLE_PAGES
    );
    const rearBound = Math.min(
      Math.max(MAX_VISIBLE_PAGES, selectedPage + MAX_VISIBLE_PAGES - 3),
      pageCount
    );
    pageButtons = getPageButtons(
      pages.slice(frontBound, rearBound),
      selectedPage,
      onclick
    );
  }
