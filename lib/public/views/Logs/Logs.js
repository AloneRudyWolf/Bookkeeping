...
export default class Overview extends Observable {
  ...
  async fetchAllLogs() {
    ...
    const params = {
      ...
      ...(this.createdFilterFrom && {
        "filter[created][from]": new Date(
          `${this.createdFilterFrom.replace(/\//g, "-")}T00:00:00.000`
        ).getTime(),
      }),
      ...(this.createdFilterTo && {
        "filter[created][to]": new Date(
          `${this.createdFilterTo.replace(/\//g, "-")}T23:59:59.999`
        ).getTime(),
      }),
    ...
    if (result.data) {
      ...
      this.logs = RemoteData.failure(
        result.errors || [{ title: result.error, detail: result.message }]
      );
    }
  ...
  async fetchOneLog(id) {
    ...
    const response = await fetchClient(`/api/logs/${id}/tree`, {
      method: "GET",
    });
    const result = await response.json();
  ...
  async createLog() {
    ...
    if (result.data) {
      ...
      await this.model.router.go(
        `/?page=log-detail&id=${this.createdLog.payload.id}`
      );
    } else {
      this.createdLog = RemoteData.failure(
        result.errors || [{ title: result.error, detail: result.message }]
      );
      this.notify();
    }
  ...
  toggleFilterExpanded(targetKey) {
    if (this.isFilterExpanded(targetKey)) {
      this.expandedFilters = this.expandedFilters.filter(
        (key) => key !== targetKey
      );
    } else {
  ...
  setCreatedFilterWithDebounce(key, date, valid) {
    ...
    this.createdFilterDebounce = setTimeout(
      () => this.setCreatedFilter(key, date, valid),
      200
    );
  }
  ...
  removeTagFromFilter(targetTag) {
    this.tagFilterValues = this.tagFilterValues.filter(
      (tag) => tag !== targetTag
    );
    this.fetchAllLogs();
  }
  ...
  getSortingPreviewOperation(key) {
    return key === this.sortingPreviewColumn
      ? this.sortingPreviewOperation
      : null;
  }
  ...
  setMarkdownBox(
    ...
    const mdBoxStyling = shouldRenderReadOnly
      ...
      : { width: "auto", height: "16rem" };

    !this.isPreviewActive &&
      setTimeout(() => {
        this.editor = setMarkDownBox(
          textAreaId,
          this.model,
          changeHandler,
          isReadOnly,
  ...
  addCollapsableColumn(rowId, name, height) {
    const existingColumn = this.collapsableColumns.find(
      (element) => element.rowId === rowId && element.name === name
    );
    if (existingColumn) {
  ...
  isColumnExpanded(rowId, name) {
    return this.collapsedColumns.some(
      (entry) => entry.rowId === rowId && entry.name === name
    );
  }
  ...
  canColumnExpand(rowId, name) {
    return this.collapsableColumns.some(
      (entry) => entry.rowId === rowId && entry.name === name && !entry.disabled
    );
  }
  ...
  getMinimalColumnHeight(rowId, name) {
    const targetColumn = this.collapsableColumns.find(
      (entry) => entry.rowId === rowId && entry.name === name
    );
